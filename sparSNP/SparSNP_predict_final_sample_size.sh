...
for i in $(awk '{ORS=" "}{print $1}' "${testset}.fam") ; do  grep "${i}_id2" "$pheno" ; done > "${pheno}_testing_ss_${test_size}"
...
plink1.9 --bfile $testset --score discovery/avg_weights_opt.score sum -out GRS_prediction_sparsnp_${test_size}
...
~/scripts/src/reshape_plink_grs.sh GRS_prediction_sparsnp_${test_size}.profile

printf "\npython regression\n"
python /projects/sysgen/users/jgrealey/embedding/src/src/pred_nets/sparsnp/grs_predict_sparsnp.py GRS_prediction_sparsnp_${test_size}.profile_formatted "$pheno""_testing_ss_${test_size}_reshape"
