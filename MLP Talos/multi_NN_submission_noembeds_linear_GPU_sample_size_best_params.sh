...
cd ~/test_embeds
pipenv shell
cd /sysgen/workspace/users/jgrealey/jgrealey/embedding/src/src/pred_nets
...
if [ "$ss" == "100k" ]; then
	...
	vali_size=2000
	test_size=40000
	echo "100k sample size with train $train_size vali $vali_size test $test_size"
	#restrict_hparams="False"
	restrict_hparams="True"
	echo "restricting hyperparams $restrict_hparams"
...
########################################################################
savestring="_predictions_on_July_2021_linear_embeddings_diffseed_nomu_patience_${patience}_samplesize_${ss}_train_${train_size}_vali_${vali_size}_test_${test_size}_restricthparams_GPU_final"
echo $savestring
echo "GPU JOBS "
#if [ 1 -eq 0 ]; then
#====================================================================================
...
if ${GPU}; then
	for i in "${emptydir[@]}"
	#for i in "${fixed_sims_med_herit_same_dist[@]}"

	#for i in "${run_diff_seed[@]}"
	do
		hparam_file=$(ls /sysgen/workspace/users/jgrealey/jgrealey/embedding/src/src/pred_nets/final_talos_rounds/ | grep "${i}")
		echo "${hparam_file}"
		#exit 1
		fullsimdir=$simdir$i
    	phendir=$(find $fullsimdir -type f -name "*phenotype_num*.csv")
		string="python -u -W ignore prediction_networks_noembeds_linear_GPU_samplesize_and_besthparams.py $train_file $vali_file $test_file $phendir  $embed "$i""$savestring" True $train_size $vali_size $test_size $hparam_file $patience $restrict_hparams"	
...
do
	...
	string2="python -u -W ignore prediction_networks_noembeds_linear_GPU_samplesize_and_besthparams.py $train_file $vali_file $test_file $phendir  $embed "$i""$savestring" False $train_size $vali_size $test_size $hparam_file $patience $restrict_hparams"	
