...
if [ ! -f "/projects/sysgen/users/jgrealey/embedding/src/src/pred_nets/grs/PRSice/sorted_bim_files/${file}_sorted_${train_size}.bim" ]; then
	echo "sorting bim file ${file}.bim "
	sort -k 2 ${file}".bim" > "/projects/sysgen/users/jgrealey/embedding/src/src/pred_nets/grs/PRSice/sorted_bim_files/$sorted_final_filtered_file_${train_size}.bim"
fi

echo  "/projects/sysgen/users/jgrealey/embedding/src/src/pred_nets/grs/PRSice/sorted_bim_files/${file}_sorted_${train_size}.bim" "${output}_gwas_sorted.qassoc" 
head  "/projects/sysgen/users/jgrealey/embedding/src/src/pred_nets/grs/PRSice/sorted_bim_files/${file}_sorted_${train_size}.bim"
head  ${output}"_gwas_sorted.qassoc"  


paste -d ' ' "/projects/sysgen/users/jgrealey/embedding/src/src/pred_nets/grs/PRSice/sorted_bim_files/$sorted_final_filtered_file_${train_size}.bim" ${output}"_gwas_sorted.qassoc" > ${output}"_gwas_sorted_input_file"
...
Rscript ../PRSice.R --dir ../ --prsice ../PRSice_linux --stat BETA --beta --binary-target F  --base ${output}"_gwas_sorted_input_file" --target ${test_file} --pheno "${pheno}_testing_ss_${test_size}_reshape" --A1 4 --bp 3 --A2 5 --snp 1 --index --pvalue 14 --stat 10 --out "PRSice_output_ss_${train_size}_${test_size}" --print-snp
...
python ../grs_predict_prsice.py "PRSice_output_ss_${train_size}_${test_size}.best" "${pheno}_testing_ss_${test_size}_reshape"
