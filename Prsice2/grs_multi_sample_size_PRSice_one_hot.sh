...
grsdir='/sysgen/workspace/users/jgrealey/jgrealey/embedding/src/src/pred_nets/grs/PRSice/one_hot'
cd $grsdir
savestring="PRSice_one_hot_9th_Feb_2021_${ss}"
#trainableembeds
saves=$((0))
#pval="1e4"
...
do
	...
	#this one
	#tail -n 20 ${ss}*.out | grep "Errno2"; 

	if tail -n 20 ${ss}*.out | grep -q "Errno"; then
		echo "$i ERROR, resubmit"
		#ls ${ss}*.outi
		JID=$(sbatch --parsable --output "${ss}_grs_$savestring.out" ~/scripts/src/grs_sample_size_PRSice_one_hot.sh ${genofile} ${phendir} ${savestring} ${testfile} ${pval}) || sleep 600 && scontrol requeue $JID
		#tail -n 20 ${ss}*.out | grep "Error"; 
		#exit
	else
		echo "$i no error"
	fi
