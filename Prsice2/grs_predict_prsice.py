...
def main():
	if len(sys.argv)!=3:
		...
		sys.exit(0)
	plot_dir="/projects/sysgen/users/jgrealey/embedding/hun_k_samples/plots/grs/"
	results_dir="/projects/sysgen/users/jgrealey/embedding/hun_k_samples/results/grs/"
	grs_file=sys.argv[1]
	pheno_file=sys.argv[2]
	print(grs_file,pheno_file)
	#	note load in GRS and split pheno file
	grs=pd.read_csv(grs_file,sep=' ',index_col=0,usecols=[0,3])#,usecols=[4])
	pheno=pd.read_csv(pheno_file,index_col=0,sep=' ',header=None,usecols=[0,2])
	num_samples=pheno.shape[0]
	print(grs.head)
	print(pheno.head)
	...
	plt.savefig(plot_dir+tail+bottom+"GRS_"+str(num_samples)+"_density.pdf",bbox_inches='tight')
	...
	with open(results_dir+"/fitting_GRS_"+str(tail)+"with_"+str(num_samples)+"on_pheno_"+str(bottom)+".csv", 'w') as f:
	...
	fig.savefig(plot_dir+tail+bottom+"_PRSICE_"+str(num_samples)+"_samples_prediction_plots.pdf", bbox_inches='tight')
	fig.show()
